...
发给 Gemini 的核心永远是问题本身（连同附带的图片/文件）。是否需要额外补背景，只看一条标准：**这条消息——包括附件——能不能自洽地说明要做什么**，不要不分场景地套模板。

- **能自洽，直接发**：`这张图片是什么？`、`看看这张图的排版怎么样`——图片本身就是完整上下文，问题也不依赖任何 Claude Code 内部信息。这类请求直接把问题和附件传给 `chat.py`，不要额外包装。
- **不能自洽，需要补充**：`帮我分析一下这个函数为什么运行很慢`——这依赖当前项目的目标、具体代码、之前排查到的线索，Gemini 网页端完全看不到，直接丢这一句问不到点子上。

**判断不了就用自检**：如果一个完全没参与过这次任务、但能看到这条消息全部内容（含附件）的人，能不能明白要做什么、为什么问、期待什么样的答案？能 → 直接发；不能 → 只补缺的那一部分，不是照抄整套模板。
...
**同一 `--session` 内的追问**：Gemini 已经具备上一轮建立的背景，通常只发【问题】。但只要这次要问的内容涉及 Claude Code 内部知道、却从未真正发给 Gemini 的新信息（新代码改动、新报错、新文件），就按上面的自检重新判断，不要假设 Gemini "应该也看到了"。

**高风险提问可选一步理解校验**：背景比较复杂时，可以先让 Gemini 用一两句话复述它理解的任务，确认无误后再展开完整分析。简单问题不需要这一步。

### 可选参数

| 参数 | 作用 | 使用场景 |
|------|------|---------|
| `-a image.png` | 附带图片 | 需要传图片时 |
...
6. **按需补背景，不无脑套模板** — Gemini 不共享 Claude Code 的上下文；问题本身能自洽就直接发，只有依赖项目/代码/历史结论时才补充，不假设对方"应该知道"，也不给简单问题强行加模板
...
Gemini 网页版一次最多接受 **10 张图片**。超过 10 张时返回 `TOO_MANY_IMAGES` 错误，不自动截取。
